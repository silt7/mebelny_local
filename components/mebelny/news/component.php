...
if(strripos($uri, '.') !== false){
    $url = 'https://'.$_SERVER[HTTP_HOST].'/'.$section.'/'.preg_replace("/\..+/", "", $uri).'/';
...
if($arResult["FOLDER"] == '/stati/' || $arResult["FOLDER"] == '/akcii/' || $arResult["FOLDER"] == '/novosti/'){
    if($arResult["FOLDER"] == '/stati/' || $arResult["FOLDER"] == '/novosti/'){
        if($arResult["FOLDER"] == '/stati/'){
            $iblockId = 1;
        }
        if($arResult["FOLDER"] == '/novosti/'){
            $iblockId = 52;
        }
        if($arResult["VARIABLES"]["ELEMENT_CODE"]){
            $code = $arResult["VARIABLES"]["ELEMENT_CODE"];
            $arFilterCodes = array();
            $arFilter = Array('IBLOCK_ID' => $iblockId, 'CODE' => $code, "ACTIVE" => "Y");
            ...
            if($arSection["ID"]){
                ...
                $arResult["VARIABLES"]["SECTION_CODE"] = $code;
            }
        }
    }
    if($arResult["FOLDER"] == '/akcii/'){
        $dir = $APPLICATION->GetCurDir();
        if($dir == "/akcii/proshli/" || $dir == "/akcii/tekuschie/") {
            $componentPage = 'section';
        }
    }
}
