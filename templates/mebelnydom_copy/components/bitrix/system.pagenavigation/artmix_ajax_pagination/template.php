...
?>
<?
// редирект на последнюю страницу, если задана большая, чем последняя
$page = $APPLICATION->GetCurPage();
if($arResult["NavPageNomer"] == 1 && $_GET["PAGEN_1"] > $arResult["NavPageCount"]){
    $url = $page . '?PAGEN_1=' . $arResult["NavPageCount"];
    header("HTTP/1.1 301 Moved Permanently");
    header("Location: $url");
    exit();
}
// редирекст с PAGEN_1=1 на первую страницу
if($_GET["PAGEN_1"] == 1){
    header("HTTP/1.1 301 Moved Permanently");
...
<div class="ax-pagination js-ax-ajax-pagination-container ax-grey">
    <div class="ax-pagination-container row">
        ...
        <?php if ($arResult['NavPageNomer'] < $arResult['NavPageCount']) { ?>
            <button class="ax-show-more-pagination js-ax-show-more-pagination" rel="nofollow"
                data-href="<?php echo $arResult['sUrlPath'] ?>?<?php echo $strNavQueryString ?>PAGEN_<?php echo $arResult['NavNum'] ?>=<?php echo ($arResult['NavPageNomer'] + 1) ?>">
        ...
        <?php } elseif ($arResult['NavPageNomer'] == $arResult['NavPageCount']) { ?>
            <span class="ax-show-more-pagination js-ax-show-more-pagination last-page" rel="nofollow">
                <?
                $page = $APPLICATION->GetCurPage();
                if(strpos($page, 'stati') > -1){
                    $str = "Показаны  все статьи";
                }else{
                    $str = "Показаны  все товары";
                }
                ?>
                <span class="ax-no-more"><?php echo $str ?></span>
            </span>
            ...
            <?php if ($arResult['bDescPageNumbering'] === true) { ?>
                ...
                <?php if ($arResult['NavPageNomer'] < $arResult['NavPageCount']) { ?>
                    <?php if ($arResult['bSavePage']) { ?>
                        ...
                        <li>
                            <a class="" href="<?php echo $arResult['sUrlPath'] ?>?<?php echo $strNavQueryString ?>PAGEN_<?php echo $arResult['NavNum'] ?>=<?php echo ($arResult['NavPageNomer'] + 1) ?>">
                                <span>1</span>
                            </a>
                        </li>
                    <?php } else { ?>
                        <?php if (($arResult['NavPageNomer'] + 1) == $arResult['NavPageCount']) { ?>
                            <li class="ax-pag-prev">
                                <a class="" href="<?php echo $arResult['sUrlPath'] ?><?php echo $strNavQueryStringFull ?>">
                                    <span><?php echo Loc::getMessage('ARTMIX_AJAXPAGINATION_BACK') ?></span>
                                </a>
                            </li>
                        <?php } else { ?>
