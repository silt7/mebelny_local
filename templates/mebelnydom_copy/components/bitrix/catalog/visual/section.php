<? if(!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED!==true)die();
   ...
   // ID верхнего раздела
    ...
    //Накрутка рейтинга для пустых разделов
...
<?
    // Признак для отсеивания товаров, которых нет в наличии
    $filter =  array(
        "LOGIC" => "AND",
            array(
                "LOGIC" => "OR",
                array('>PROPERTY_POD_ZAKAZ' => 0),
                array('>PROPERTY_QTY' => 0)
            )
    );
    global $arrFilter;
    $arrFilter = $filter;
    //END Признак для отсеивания товаров, которых нет в наличии
?>
...
<section class="catalog">
    ...
    <div class="container catalog__wrap">
        <div id="filter" class="catalog__filter filter">
            ...
            <?
            if( ((stripos($_SERVER['REQUEST_URI'], 'filter') === false) ) and ($arResult['VARIABLES']['IBLOCK_SECTION_ID'] == '') ){
                $APPLICATION->IncludeComponent(
                    "bitrix:catalog.section.list", 
                    "RelinkTagsMobile2", 
                    array(
