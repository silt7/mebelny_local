...
if (count($arNaborAll) > 0 && $arNaborAll[0] > 0) {
	...
	foreach ($arResult["PRICES"] as $code=>$arPrice) {
    	...
    	$arResult["PRICES"][$code]['VALUE'] = $price;
    	$arResult["PRICES"][$code]['PRINT_VALUE_VAT'] = number_format($price, 0, ',', ' ') . ' руб.';
    	$arResult["PRICES"][$code]['DISCOUNT_VALUE'] = $discount_price;
	}
...
if(CModule::IncludeModule("askaron.reviews"))
{
    ...
    {
        $arReviews[] = $arFields;
        $review_avg += $arFields['GRADE'];
    }
    if($review_avg != 0){
        $arResult['review_avg'] = round($review_avg/count($arReviews),1);
    } else {
        $arResult['review_avg'] = 0;
    }
...
//echo$_SESSION["CATALOG_COMPARE_LIST"];
/*if(count($_SESSION["CATALOG_COMPARE_LIST"][$arResult['IBLOCK_ID']]["ITEMS"]) == 0){
    $arResult["COMPARE_COUNT"] = 'нет';
} else {
    $arResult["COMPARE_COUNT"] = count($_SESSION["CATALOG_COMPARE_LIST"][$arResult['IBLOCK_ID']]["ITEMS"]);
}*/
