	...
	<form id="f_feedback_<?=$ALX?>" class="products__findcheap_form" name="f_feedback_<?=$ALX?>" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
...
?>
				<? /*?><span id="afbf_<?=mb_strtolower($arField["CODE"])?>" class="afbf_item_pole<?=$fieldClass?>"> <? */?>
<?				/*LIST*/?>
<?					if($arField["TYPE"] == "L"):?>
...
<?							if(!empty($_POST["FIELDS"][$arField["CODE"]])):?>
								<input type="text" size="40" id="<?=$arField["CODE"]?>1" name="FIELDS[<?=$arField["CODE"]?>]" value="<?=trim(htmlspecialcharsEx($_POST["FIELDS"][$arField["CODE"]]))?>" class="afbf_inputtext" readonly onclick="BX.calendar({node:this,field:'FIELDS[<?=$arField["CODE"]?>]',form:'',bTime:<?=$bShowTime?>,currentTime:'<?=(time()+date("Z")+CTimeZone::GetOffset())?>',bHideTime:<?=$bHideTime?>});" />
<?							else:?>
								<input type="text" size="40" id="<?=$arField["CODE"]?>1" name="FIELDS[<?=$arField["CODE"]?>]" value="<?=$arField["DEFAULT_VALUE"]?>" class="afbf_inputtext" readonly onclick="BX.calendar({node:this,field:'FIELDS[<?=$arField["CODE"]?>]',form:'',bTime:<?=$bShowTime?>,currentTime:'<?=(time()+date("Z")+CTimeZone::GetOffset())?>',bHideTime:<?=$bHideTime?>});" />
<?							endif;?>
...
<?						if(!empty($_POST["FIELDS"][$arField["CODE"]])):?>
							<input type="text" id="<?=$arField["CODE"]?>1" name="FIELDS[<?=$arField["CODE"]?>]" value="<?=trim(htmlspecialcharsEx($_POST["FIELDS"][$arField["CODE"]]))?>" class="afbf_inputtext" />
<?						elseif(!empty($arField["AUTOCOMPLETE_VALUE"])):
							$readonly = "";
							...
							<input type="text" id="<?=$arField["CODE"]?>1" name="FIELDS[<?=$arField["CODE"]?>]" <?=$readonly?>value="<?=trim(htmlspecialcharsEx($arField["AUTOCOMPLETE_VALUE"]))?>" placeholder="<?=$arField["NAME"]?>"/>
<?						else:?>
                        <? if($arField["CODE"] == "TSMB_FID1") { $f_value = $arParams["LINK_TOV"]; } else { $f_value = $arField["DEFAULT_VALUE"]; } ?>
							<input type="text" <? if($arField["CODE"] == "TSMB_FID1") { ?>style=" display:none;" <? } ?>id="<?=$arField["CODE"]?>1" name="FIELDS[<?=$arField["CODE"]?>]" value="<?=$f_value;?>" onblur="if(this.value==''){this.value='<?=$arField["DEFAULT_VALUE"]?>'}" onclick="if(this.value=='<?=$arField["DEFAULT_VALUE"]?>'){this.value=''}" placeholder="<?=$arField["NAME"]?>"/>
<?						endif;?>
...
<?					if($arField["MULTIPLE"] == "Y"):?>
						<input type="file" id="afbf_file_input_add<?=$k?>" name="myFile[<?=$arField['CODE']?>][]" class="afbf_file_input_add" multiple />
<?					else:?>
...
<?		echo bitrix_sessid_post()?>
    	<div class="products__btn_wrap"><button type="submit" class="" id="fb_close_<?=$ALX?>" name="SEND_FORM"><?=GetMessage('ALX_TP_MESSAGE_SUBMIT')?> ></button></div>
	</form>