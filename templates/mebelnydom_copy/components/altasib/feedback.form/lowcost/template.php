...
<?		if ($arParams['ALX_LINK_POPUP'] !== 'Y'):?>
		...
		<span id="afbf_err_<?=$ALX?>" class="alx-feedb-error" style="color:#ef617f; display:block; width:100%; margin-bottom:10px !important;"></span>
<?		endif?>
<?
...
<?		if (!is_array($arParams['INPUT_APPEARENCE'])): ?>
			<span class="alx_feed_back <? $arParams['INPUT_APPEARENCE'] != 'DEFAULT' ? strtolower($arParams['INPUT_APPEARENCE']) : '' ?>" id="alx_feed_back_<?=$ALX ?>">
<?		else: ?>
			<div class="alx_feed_back <? foreach ($arParams['INPUT_APPEARENCE'] as $param) if ($param != 'DEFAULT') echo strtolower($param) . ' ' ?>" id="alx_feed_back_<?=$ALX ?>">
<?		endif ?>
<?
?>
...
<?
	...
	<? if (count($arResult["FORM_ERRORS"]) == 0 && ($arResult["success_" . $ALX] == "yes" || $_REQUEST["success_" . $ALX] == "yes")): ?>
		<span class="afbf_success_block<? if ($arParams['ALX_LINK_POPUP'] !== 'Y'):?> _without-popup<? endif;?>">
				<div class="mess" style="color:#0097f6;"><?=$arParams["MESSAGE_OK"]; ?></div>
		</span>
		<? if ($arParams['ALX_LINK_POPUP'] == 'Y'):?>
...
<style>#alx_feed_back_<?=$ALX?>.alx_feed_back, #afbf_err_<?=$ALX?>.alx-feedb-error{width:<?=$arParams["WIDTH_FORM"]?>;}</style>