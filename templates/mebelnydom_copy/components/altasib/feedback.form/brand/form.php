	...
	<form id="f_feedback_<?=$ALX?>" name="f_feedback_<?=$ALX?>" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data" class="form-help__form">
...
<?							if(!empty($_POST["FIELDS"][$arField["CODE"]])):?>
								<input type="text" id="<?=$arField["CODE"]?>1" name="FIELDS[<?=$arField["CODE"]?>]" value="<?=trim(htmlspecialcharsEx($_POST["FIELDS"][$arField["CODE"]]))?>" class="afbf_inputtext p-company__form-input" readonly onclick="BX.calendar({node:this,field:'FIELDS[<?=$arField["CODE"]?>]',form:'',bTime:<?=$bShowTime?>,currentTime:'<?=(time()+date("Z")+CTimeZone::GetOffset())?>',bHideTime:<?=$bHideTime?>});" />
<?							else:?>
								<input type="text" id="<?=$arField["CODE"]?>1" name="FIELDS[<?=$arField["CODE"]?>]" value="<?=$arField["DEFAULT_VALUE"]?>" class="afbf_inputtext p-company__form-input" readonly onclick="BX.calendar({node:this,field:'FIELDS[<?=$arField["CODE"]?>]',form:'',bTime:<?=$bShowTime?>,currentTime:'<?=(time()+date("Z")+CTimeZone::GetOffset())?>',bHideTime:<?=$bHideTime?>});" />
<?							endif;?>
...
<?						if(!empty($_POST["FIELDS"][$arField["CODE"]])):?>
							<input type="text" id="<?=$arField["CODE"]?>1" name="FIELDS[<?=$arField["CODE"]?>]" value="<?=trim(htmlspecialcharsEx($_POST["FIELDS"][$arField["CODE"]]))?>" class="afbf_inputtext p-company__form-input" />
<?						elseif(!empty($arField["AUTOCOMPLETE_VALUE"])):
							$readonly = "";
							...
							<input type="text" id="<?=$arField["CODE"]?>1" name="FIELDS[<?=$arField["CODE"]?>]" <?=$readonly?>value="<?=trim(htmlspecialcharsEx($arField["AUTOCOMPLETE_VALUE"]))?>" placeholder="<?=$arField["NAME"]?>" class="p-company__form-input"/>
<?						else:?>
                            <label for="tel" class="form-help__col">
                              <span class="form-help__label"><?=$arField["NAME"]?></span>
                              <input type="text" id="<?=$arField["CODE"]?>1" name="FIELDS[<?=$arField["CODE"]?>]" value="<?=$arField["DEFAULT_VALUE"]?>" onblur="if(this.value==''){this.value='<?=$arField["DEFAULT_VALUE"]?>'}" class="form-help__input" onclick="if(this.value=='<?=$arField["DEFAULT_VALUE"]?>'){this.value=''}"/>
                            </label>
...
<?					endif;?>
						<div class="afbf_input_group">
							<input type="text" id="<?=$arField["CODE"]?>1" name="FIELDS[myFile][<?=$arField["CODE"]?>]" value="" class="afbf_inputtext" />
...
<?		echo bitrix_sessid_post()?>
        <div class="form-help__col">
          <button type="submit" class="form-help__btn" id="fb_close_<?=$ALX?>" name="SEND_FORM"  /><?=GetMessage('ALX_TP_MESSAGE_SUBMIT')?></button>
        </div>
