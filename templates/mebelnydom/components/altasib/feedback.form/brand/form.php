	...
	<form id="f_feedback_<?=$ALX?>" name="f_feedback_<?=$ALX?>" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data" class="p-company__form">
...
<?						else:?>
							<input type="text" id="<?=$arField["CODE"]?>1" name="FIELDS[<?=$arField["CODE"]?>]" value="<?=$arField["DEFAULT_VALUE"]?>" onblur="if(this.value==''){this.value='<?=$arField["DEFAULT_VALUE"]?>'}" class="p-company__form-input" onclick="if(this.value=='<?=$arField["DEFAULT_VALUE"]?>'){this.value=''}" placeholder="<?=$arField["NAME"]?>"/>
<?						endif;?>
...
<?		echo bitrix_sessid_post()?>
    	<button type="submit" class="p-company__form-sbm" id="fb_close_<?=$ALX?>" name="SEND_FORM"  /><?=GetMessage('ALX_TP_MESSAGE_SUBMIT')?></button>
	</div>
    </form>