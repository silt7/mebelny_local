    ...
    $.KomboxSmartFilter.prototype = {
        ...
        showModef: function (result)
        {
            ...
            if(modef_num.length)
            {
                ...
                var sectionLink = '';
                if(countSections == 1){
                    // проверяем, чтобы в текущей ссылке после filter был один элемент
                    var href = $('#modef a').attr('href');
                    // в сслылках на разделы ищем такой же атрибут
                    $('.w1510_menuTags_item').each(function () {
                       if($(this).attr('data-filter') == href){
                          ...
                          if(sectionLink){
                              ...
                              modef.show();
                              $('#modef a').attr('href', sectionLink);
                              $('#set_filter').hide();
                              if($('.bx-touch').length > 0){
                                  $('#set_links_filter').attr('href', sectionLink).css('display', 'block');
                              }else{
                                  $('#set_links_filter').show().attr('href', sectionLink);
                              }
                          }
                       }
                    });
