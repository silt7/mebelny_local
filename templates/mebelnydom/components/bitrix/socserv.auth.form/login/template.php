...
{
	var content = BX("bx_socserv_form_login_"+id);
	if(content)
	{
...
?>
		<li>
			<a id="bx_socserv_icon_login_<?=$service["ID"]?>" class="<?=\Bitrix\Main\Text\HtmlFilter::encode($service["ICON"])?> bx-authform-social-icon" href="javascript:void(0)" onclick="<?=\Bitrix\Main\Text\HtmlFilter::encode($onclick)?>" title="<?=\Bitrix\Main\Text\HtmlFilter::encode($service["NAME"])?>"></a>
	<?if($service["ONCLICK"] == '' && $service["FORM_HTML"] <> ''):?>
			<div id="bx_socserv_form_login_<?=$service["ID"]?>" class="bx-authform-social-popup">
