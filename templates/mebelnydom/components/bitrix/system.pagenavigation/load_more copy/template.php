...
// Редирект на последнюю существующую страницу пагинации, при вводе несуществующей.
if($arResult['NavPageCount']<$_GET['PAGEN_1']){
	$redirectPath = $arResult['sUrlPath'].'?PAGEN_1='.$arResult["NavPageCount"];
	LocalRedirect($redirectPath, false, '301 Moved permanently');
}
...
if($arResult["bDescPageNumbering"] === true)
{
	...
	?>
			<a class="nav-current-page"><?=$NavRecordGroupPrint?></a>
	<?
...
else
{
	...
	do
	{
		if ($arResult["nStartPage"] <= 2 || $arResult["nEndPage"]-$arResult["nStartPage"] <= 1 || abs($arResult['nStartPage']-$arResult["NavPageNomer"])<=2)
		{
			...
			if ($arResult["nStartPage"] == $arResult["NavPageNomer"]):
	?>
			<a class="nav-current-page"><?=$arResult["nStartPage"]?></a>
	<?
...
<?php if ($arResult['NavPageNomer'] < $arResult['NavPageCount']) { ?>
    <a class="ax-show-more-pagination js-ax-show-more-pagination" rel="nofollow"
       href="<?php echo $arResult['sUrlPath'] ?>?<?php echo $strNavQueryString ?>PAGEN_<?php echo $arResult['NavNum'] ?>=<?php echo ($arResult['NavPageNomer'] + 1) ?>">
        <div class="load_more"><span class="ax-link ax-dotted-link" style="color: black;"><?php echo "Показать ещё" ?></span>
    </a>
<?php } elseif ($arResult['NavPageNomer'] == $arResult['NavPageCount']) { ?>
    <span class="ax-show-more-pagination js-ax-show-more-pagination last-page" rel="nofollow"></div>
        <?
        ...
        ?>
        <div class="load_more"><span class="ax-no-more"><?php echo "Загружено все" ?></span></div>
    </span>
