	...
	BX.Sale.OrderAjaxComponent = {
		...
		getBlockFooter: function(node)
		{
			...
			if (currentSection && currentSection.id.indexOf(firstSection.id) == '-1')
			{
				buttons.push(
					BX.create('A', {
						props: {
							href: '',
							className: 'pull-left btn btn-default btn-md btn-buy'
						},
			...
			if (!isLastNode)
			{
				buttons.push(
					BX.create('A', {
						props: {href: '', className: 'pull-right btn btn-default btn-buy btn-md'},
	...
	var Order = new Vue({
		...
		data: function(){
			return {
				...
				// Параметры для генерации представлений
				description: null,
				url: null,
				siteId: null,
				delivery:[],
				...
				is_load: false,
				search_city_text: '',
				// Кастомная поля адресов
				custom_delivery: [
					{
						id: 1,
						active: true,
						img: "/upload/medialibrary/1a1/image-13.png",
						name: "Деловые линии"
					},
					{
						id: 2,
						active: false,
						img: "/upload/medialibrary/c17/image-12.png",
						name: "ПЭК"
					},
					{
						id: 3,
						active: false,
						img: "/upload/medialibrary/884/Group-707.png",
						name: "ТК на выбор"
					},
				],
				custom_fields: [
					...
					},
					{
						...
						items: [
							{
								id: 5,
								name: "Заберу сам в пункте самовывоза",
								img: null,
								active: true,
								fields: []
							}
		...
		methods: {
			deleteCityText(){
				var app = this;
				app.search_city_text = '';

			},
			generateData(){
				var data = {};
				$('#bx-soa-inputs').find ('input, textearea, select').each(function() {
			...
			setPaySystem(item){
				var app = this;
				if(app.checkPaySystemDisabled(item)){
					return;
				}
				for(var key in app.pay_system){
					app.pay_system[key].CHECKED = 'N';
					if(app.pay_system[key].ID == item.ID){
						app.pay_system[key].CHECKED = 'Y';
					}
				}
				app.$forceUpdate();
				return true;
			},
			checkPaySystemDisabled(item){
				var app = this;
				var delivery = app.deliveryValue();
				if(delivery == 46 || delivery == 47 || delivery == 48){
			...
			setRadio(item_id){
				var app = this;
				for(var key in app.custom_fields){
					if(app.custom_fields[key].delivery_id == app.delivery_item.ID){
						for(var l in app.custom_fields[key].items){
							app.custom_fields[key].items[l].active = false;
							if(app.custom_fields[key].items[l].id == item_id){
								app.custom_fields[key].items[l].active = true;
							}
						}
						break;
					}
			...
			setDelivery(item){
				var app = this;
				for(var key in app.delivery){
					app.delivery[key].CHECKED = 'N';
					if(app.delivery[key].ID == item.ID){
						app.delivery[key].CHECKED = 'Y';
						app.delivery_item = app.delivery[key];
						app.delivery_item.CUSTOM_FIELDS = null;
						for(var l in app.custom_fields){
							if(app.custom_fields[l].delivery_id == app.delivery_item.ID){
								app.delivery_item.CUSTOM_FIELDS = app.custom_fields[l];
								break;
							}
						}
					}
			...
			paySystemValue(){
				var app = this;
				for(var key in app.pay_system){
					if(app.pay_system[key].CHECKED != undefined && app.pay_system[key].CHECKED == 'Y'){
						return app.pay_system[key].ID;
					}
				}
				return null;
			},
			deliveryValue(){
				var app = this;
				for(var key in app.delivery){
					if(app.delivery[key].CHECKED != undefined && app.delivery[key].CHECKED == 'Y'){
						if(app.delivery[key].ID == '4'){
							for(var l in app.custom_delivery){
								if(app.custom_delivery[l].active){
									return app.custom_delivery[l].id;
								}
							}
						}
						else{
							return app.delivery[key].ID;
						}
					}
				}
				return null;
			},
			checkActiveClass(step_id){
				var app = this;
				var active = false
				for(var key in app.steps){
					if(step_id == app.steps[key].id && !active){
						return true;
					}
					if(app.steps[key].active){
						active = true;
					}
			...
			setCustomDelivery(del_id){
				var app = this;
				for(var key in app.custom_delivery){
					app.custom_delivery[key].active = false;
					if(app.custom_delivery[key].id == del_id){
						app.custom_delivery[key].active = true;
					}
				}
			},
			setStep(type){
				...
				for(var key in app.steps){
					if(app.steps[key].active){
						if(type == 'next'){
							if(!app.validate(app.steps[key].id)){
								return false;
							}
							var next_key = parseInt(key) + 1;
							if(app.steps[next_key] != undefined){
								app.steps[next_key].active = true;
							}
						}
						else if(type == 'prev'){
							var prev_key = parseInt(key) - 1;
							if(app.steps[prev_key] != undefined){
								app.steps[prev_key].active = true;
							}
						}
						app.steps[key].active = false;
						var scrollTop = $('#main-steps').offset().top;
						$(document).scrollTop(scrollTop);

						
						var delivery = app.deliveryValue();
						if(delivery == 46 || delivery == 47 || delivery == 48){
							for(var l in app.pay_system){
								if(app.pay_system[l].ID == '10'){
									app.setPaySystem(app.pay_system[l]);
									break;
								}
							}
						}
			...
			checkCheckbox(){
				...
				for(var key in app.properties){
					if(app.properties[key].TYPE == "Y/N"){
						return true;
					}
				}
				return false;
			...
			validateStep1(){
				...
				var result = true;
				for(var key in app.properties){
					app.properties[key].ERROR = false;
					if(app.properties[key].ERROR_TEXT != undefined){
						delete(app.properties[key].ERROR_TEXT);
					}
					if(app.properties[key].IS_PHONE == 'Y' && app.properties[key].REQUIRED == 'Y'){
						var re = /^(\s*)?(\+)?([- _():=+]?\d[- _():=+]?){10,14}(\s*)?$/;
						if(!re.test(app.properties[key].VALUE)){
							app.properties[key].ERROR = true;
							app.properties[key].ERROR_TEXT = 'Номер телефона введен неверно';
							result = false;
						}
					}
					else if(app.properties[key].IS_EMAIL == 'Y' && app.properties[key].REQUIRED == 'Y'){
						var re = /^[\w-\.]+@[\w-]+\.[a-z]{2,4}$/i;
						if(!re.test(app.properties[key].VALUE)){
							app.properties[key].ERROR = true;
							app.properties[key].ERROR_TEXT = 'E-mail введен неверно';
							result = false;
						}
					}
					else{
						if(app.properties[key].PROPS_GROUP_ID == '1' && app.properties[key].REQUIRED == 'Y' && app.properties[key].VALUE == ''){
							app.properties[key].ERROR = true;
							result = false;
						}	
					}
			...
			validateStep2(){
				...
				for(var key in app.properties){
					app.properties[key].ERROR = false;
					if(app.delivery_item != null && app.delivery_item.ID != "2" && app.properties[key].PROPS_GROUP_ID == '2' && app.properties[key].REQUIRED == 'Y' && app.properties[key].VALUE == ''){
						app.properties[key].ERROR = true;
						result = false;
					}
				}
				if(app.delivery_item != null){
					for(var key in app.custom_fields){
						if(app.custom_fields[key].delivery_id == app.delivery_item.ID){
							for(var l in app.custom_fields[key].items){
								if(app.custom_fields[key].items[l].active){
									for(var f in app.custom_fields[key].items[l].fields){
										app.custom_fields[key].items[l].fields[f].ERROR = false;
										if(app.custom_fields[key].items[l].fields[f].REQUIRED == 'Y'){
											if(app.custom_fields[key].items[l].fields[f].VALUE == '' || app.custom_fields[key].items[l].fields[f].VALUE == null){
												app.custom_fields[key].items[l].fields[f].ERROR = true;
												result = false;
											}
										}
									}
			...
			validate(step_id = null){
				var app = this;
				if(step_id != null){
					if(step_id == 1){
						return app.validateStep1();
					}
					else if(step_id == 2){
						return app.validateStep2();
					}
					else if(step_id == 3){
						app.submit();
						return false;
					...
					if(!app.validateStep1() || !app.validateStep2() || app.active_step.id != 3){
						return false;
					}
					return true;
				}
			},
			setFieldCheckbox(field){
				if(field.VALUE == null || field.VALUE == 'N'){
					field.VALUE = 'Y';
				}
				else{
					field.VALUE = 'N';	
			...
			generateMainParamters(parametrs){
				var app = this;
				var result = null;
				if(app.parametrs.result != undefined){
					var result = app.parametrs.result;
				}
				if(parametrs.citys != undefined){
					app.citys = parametrs.citys;
				}
				if(result.ORDER_PROP != undefined && result.ORDER_PROP.properties != undefined){
					app.properties = [];
					for(var key in result.ORDER_PROP.properties){
						var value = null;
						var property = result.ORDER_PROP.properties[key];
				...
				if(result.PAY_SYSTEM != undefined){
					app.pay_system = [];
					for(var key in result.PAY_SYSTEM){
						app.pay_system.push(result.PAY_SYSTEM[key]);
					}
				}
				if(result.TOTAL != undefined){
					app.total = result.TOTAL;
				}
				if(result.STORE_LIST != undefined){
					app.store_list = [];
					for(var key in result.STORE_LIST){
						app.store_list.push(result.STORE_LIST[key]);
					}
				}
				if(result.GRID != undefined && result.GRID.ROWS){
					app.products_in_cart = [];
					for(var key in result.GRID.ROWS){
						app.products_in_cart.push(result.GRID.ROWS[key]);
					}
				}
				if(app.parametrs.ajaxUrl != undefined){
					app.url = app.parametrs.ajaxUrl;
				}
				if(app.parametrs.siteId != undefined){
					app.siteId = app.parametrs.siteId;
				}
				if(result.DELIVERY != undefined){
					app.delivery = [];
					app.custom_delivery = [];
					for(var key in result.DELIVERY){
						if(result.DELIVERY[key].ID == '1' || result.DELIVERY[key].ID == '2' || result.DELIVERY[key].ID == '4'){
							var delivery = result.DELIVERY[key];
							if(app.delivery.length == 0){
								delivery.CHECKED = 'Y'
							}
							if(delivery.CHECKED == undefined){
								delivery.CHECKED = 'N';
							}
							if(delivery.CHECKED == 'Y'){
								app.delivery_item = delivery;
								app.delivery_item.CUSTOM_FIELDS = null;
								for(var l in app.custom_fields){
									if(app.custom_fields[l].delivery_id == app.delivery_item.ID){
										app.delivery_item.CUSTOM_FIELDS = app.custom_fields[l];
							...
							var shops = [];
							for(var l in delivery.STORE){
								for(var k in app.store_list){
									if(delivery.STORE[l] == app.store_list[k].ID){
										shops.push(app.store_list[k]);
									}
								}
							}
							delivery.STORE = shops;
							app.delivery.push(result.DELIVERY[key]);
						}
						else{
							var custom_delivery = {
								id: parseInt(result.DELIVERY[key].ID),
								active: ( app.custom_delivery.length == 0 ? true : false ),
								img: result.DELIVERY[key].LOGOTIP_SRC_ORIGINAL,
								name: result.DELIVERY[key].NAME	
							};
							app.custom_delivery.push(custom_delivery);
						}
					}
				}
				app.$forceUpdate();
			...
			submit(){
				var app = this;
				if(!app.main_validate){
					console.log('No validate');
					return;
				}
				var data = app.generateData();
				var url = $('#bx-soa-order-form').attr('action');
				app.is_load = true;
				// $('#bx-soa-order-form').submit();
				// return;
                gtag('event', 'pokupka_ga');
				BX.ajax({
					timeout: 60,
					method: 'POST',
					dataType: 'json',
					...
					onsuccess: BX.proxy(function(result) {
						...
						for(var key in app.steps){
							app.steps[key].active = false;
							if(app.steps[key].id == 4){
								app.steps[key].active = true;
							}
						}
						ym(26789943,'reachGoal','pokupka');
						app.is_load = false;
					}, this),
					onfailure: BX.proxy(function(){
			...
			refreshOrder(){
				...
				var data = BX.Sale.OrderAjaxComponent.getData();
				data.action = 'refreshOrderAjax';
				data.order = app.generateData();
				data.order.DELIVERY_ID = app.deliveryValue();
				var url = $('#bx-soa-order-form').attr('action');
				...
				BX.ajax({
					...
					data: data,
					onsuccess: BX.proxy(function(result) {
						app.total = result.order.TOTAL;
						app.$forceUpdate();
						// app.parametrs = result.order;
						// generateMainParamters(app.parametrs);
					}, this),
					onfailure: BX.proxy(function(){
			...
			searchCity(item){
				var app = this;
				if(item.CODE == 'CITY'){
					app.search_city_text = item.VALUE;
				}
			},
			init(){
				var app = this;
			    ...
			    $('input[type=phone]').mask('+0 (000) 000 00 00');
			    app.parametrs = Parametrs;

			    this.generateMainParamters(Parametrs);
				BX.Sale.OrderAjaxComponent.init(Parametrs);
			},
			update(){
		...
		computed: {
				...
				if(app.delivery_item != null){
					for(var key in app.custom_fields){
						if(app.custom_fields[key].delivery_id == app.delivery_item.ID){
							for(var l in app.custom_fields[key].items){
								if(app.custom_fields[key].items[l].active){
									return app.custom_fields[key].items[l];
								}
							}
							break;
						}
			...
			search_citys: function(){
				var app = this;
				var result = [];
				if(app.search_city_text.length > 2){
					for(var key in app.citys){
						if(app.citys[key].indexOf(app.search_city_text) >= 0){
							result.push(app.citys[key]);
						}
					}
				}
				return result;
			},
			main_validate: function(){
				var app = this;
				if(!app.validate()){
					return false;
				}
		...
		watch: {
			custom_field: {
				handler: function(custom_field){
					var app = this;
					var txt = custom_field.name + '. ';
					
					if(app.delivery_item != null && app.delivery_item.ID == '4'){
						for(var key in app.custom_delivery){
							if(app.custom_delivery[key].active){
								txt = txt + app.custom_delivery[key].name + '. ';
								break;
							}
						}
					}

					for(var key in app.custom_field.fields){
						if(app.custom_field.fields[key].VALUE != null && app.custom_field.fields[key].VALUE != ''){
							txt = txt + app.custom_field.fields[key].NAME + ': ' + app.custom_field.fields[key].VALUE + '. ';
						}
					}
					app.properties.find(item => item.CODE == 'ADDRESS').VALUE = txt;3
				},
				deep: true
			},
			custom_delivery: {
				handler: function(custom_delivery){
					var app = this;
					app.refreshOrder();
				},
				deep: true
			...
			delivery_item: {
				handler: function(delivery_item){
					var app = this;
					app.refreshOrder();
				},
				deep: true
