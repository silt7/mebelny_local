			...
			<div v-for="(step, key) in steps" class="__step" v-if="step.active">
				...
				<div class="__foot_box">
					<div class="row">
						...
						<div class="col-md-6">
							<div class="text-right">
							    <a v-if="(key + 1) == steps.length - 1" class="__next" @click="setStep('next')" href="javascript:void(0)" onClick="gtag('event', 'pokupka_ga')" style="background: #3983DF;">Оформить заказ</a>
								<a v-else-if="(key + 1) != steps.length" class="__next" @click="setStep('next')" href="javascript:void(0)">Вперед</a>
							</div>
						</div>
						<div class="clearfix"></div>
					</div>
