...
Asset::getInstance()->addJs("/local/templates/mebelnydom/components/bitrix/sale.order.payment.change/.default/script.js");
Asset::getInstance()->addCss("/local/templates/mebelnydom/components/bitrix/sale.order.payment.change/.default/style.css");
...
else
{
	...
	<div class="row col-md-12 col-sm-12">
		...
		{
			?>
			<a class="sale-order-history-link btn btn-default" href="<?=$APPLICATION->GetCurPageParam("filter_history=Y", $clearFromLink, false)?>">
		...
		{
			?>
			<a class="sale-order-history-link btn btn-default" href="<?=$APPLICATION->GetCurPageParam("", $clearFromLink, false)?>">
			...
			{
				?>
				<a class="sale-order-history-link btn btn-default" href="<?=$APPLICATION->GetCurPageParam("filter_history=Y&show_canceled=Y", $clearFromLink, false)?>">
	...
	if ($_REQUEST["filter_history"] !== 'Y')
	{
		...
		foreach ($arResult['ORDERS'] as $key => $order)
		{
			...
			?>
			<div class="col-md-12 col-sm-12 sale-order-list-container" style="border:1px solid #e7eff1 !important; margin-bottom:10px;">
				<div class="row">
						...
						foreach ($order['PAYMENT'] as $payment)
						{
							...
							<div class="row sale-order-list-inner-row">
								...
								<div class="sale-order-list-inner-row-body">
									<div class="col-md-9 col-sm-8 col-xs-12 sale-order-list-payment">
										<div class="sale-order-list-payment-title">
											...
											{
												?>
												<div class="_alert"><span class="sale-order-list-status-success"><?=Loc::getMessage('SPOL_TPL_PAID')?></span></div>
												<?
											}
											else
											{
												?>
												<div class="_alert"><span class="sale-order-list-status-alert"><?=Loc::getMessage('SPOL_TPL_NOTPAID')?></span></div>
												<?
											}
											?>
										</div>
									...
									if ($payment['PAID'] === 'N' && $payment['IS_CASH'] !== 'Y')
									{
										if ($payment['NEW_WINDOW'] === 'Y')
										{
											...
											<div class="col-md-3 col-sm-4 col-xs-12 sale-order-list-button-container">
												<a class="sale-order-list-button" href="<?=htmlspecialcharsbx($payment['PSA_ACTION_FILE'])?>">
										...
										else
										{
											?>
											<div class="col-md-3 col-sm-4 col-xs-12 sale-order-list-button-container">
												<a class="btn-def ajax_reload" href="<?=htmlspecialcharsbx($payment['PSA_ACTION_FILE'])?>">
													<?=Loc::getMessage('SPOL_TPL_PAY')?>
												</a>
											</div>
											<?
								...
								<div class="col-lg-9 col-md-9 col-sm-10 col-xs-12 sale-order-list-inner-row-template">
									<a class="sale-order-list-cancel-payment btn btn-default">
							...
							<div class="row sale-order-list-inner-row">
								...
								<div class="col-md-9 col-sm-8 col-xs-12 sale-order-list-shipment">
									...
									<span class="sale-order-list-shipment-element">
										...
										{
											?>
											<div class="_alert"><span class="sale-order-list-status-success"><?=Loc::getMessage('SPOL_TPL_LOADED');?></span></div>
											<?
										}
										else
										{
											?>
											<div class="_alert"><span class="sale-order-list-status-alert"><?=Loc::getMessage('SPOL_TPL_NOTLOADED');?></span></div>
											<?
										}
										?>
									</div>
								...
								if (strlen($shipment['TRACKING_URL']) > 0)
								{
									...
									<div class="col-md-2 col-md-offset-1 col-sm-12 sale-order-list-shipment-button-container">
										<a class="sale-order-list-shipment-button" href="<?=$shipment['TRACKING_URL']?>">
			...
			<div class="col-md-12 col-sm-12 sale-order-list-container">
				<div class="row">
					<div class="col-md-12 col-sm-12 sale-order-list-accomplished-title-container">
						<div class="row">
							<div class="col-md-8 col-sm-12 sale-order-list-accomplished-title-container" style="padding-bottom:10px;">
								<h2 class="sale-order-list-accomplished-title" style="margin:15px 0 0 0;">
									<?= Loc::getMessage('SPOL_TPL_ORDER') ?>
